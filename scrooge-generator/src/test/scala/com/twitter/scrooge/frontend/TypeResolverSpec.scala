    ...
    "resolve a typedef from an included scope" in {
      ...
      resolvedDoc.defs(0) match {
        case Struct(_, _, fields, _, annotations) => {
          fields(0) match {
            case Field(1, _, _, ListType(StructType(_, Some(SimpleID("typedef1"))), None), _, _, _, _, _) => // pass
            case _ => fail()
          }
          fields(1) match {
            case Field(2, _, _, SetType(StructType(_, Some(SimpleID("typedef1"))), None), _, _, _, _, _) => // pass
            case _ => fail()
          }
